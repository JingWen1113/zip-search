...
class zipContainer extends Component {
  ...
  onChange = (event) => {
    //sets the Zip state to that of the one user entered
    this.setState({ Zip: event.target.value });
  };

  fetchLocations = async () => {
    //calls api after the zip state is updated this way it can get information for specific zip address
    try {
      let zipGiven = this.state.Zip;
      console.log("this is the zip: " + zipGiven);
      let response = await fetch(
        "http://ctp-zip-api.herokuapp.com/zip/" + zipGiven,
        { method: "GET" }
      );
